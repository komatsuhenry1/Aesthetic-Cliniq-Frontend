...
import { getProfessionalAppointmentsByDate } from "@/services/api/appointment";
...
function getAppointmentClasses(status: string) {
  if (status === "concluido") {
...
export default function ProfissionaisPage() {
  ...
  const [scheduleAppointments, setScheduleAppointments] = useState<AgendaAppointment[]>([]);
  const [scheduleAppointmentsLoading, setScheduleAppointmentsLoading] = useState(false);
  ...
  const trimmedProfessionalEmail = newProfessionalEmail.trim();
  const isNewProfessionalEmailValid = emailPattern.test(trimmedProfessionalEmail);
  const canSaveNewProfessional =
    newProfessionalName.trim().length > 0 &&
    newProfessionalSpecialty.trim().length > 0 &&
    trimmedProfessionalEmail.length > 0 &&
  ...
  useEffect(() => {
    if (!isScheduleModalOpen || !selectedProfessionalForSchedule || !scheduleSelectedDate) {
    ...
    let cancelled = false;
    async function load() {
      setScheduleAppointmentsLoading(true);
      try {
        ...
        if (!cancelled) {
          setScheduleAppointmentsLoading(false);
        }
      }
    }
    void load();
  ...
  }, [isScheduleModalOpen, selectedProfessionalForSchedule, scheduleSelectedDate]);
    ...
    try {
      if (drawerMode === "create") {
        ...
        if (!editingProfessionalId) {
          throw new Error("ID do profissional para edição não encontrado.");
        }
        const { message } = await updateProfessional({
          id: editingProfessionalId,
          name: newProfessionalName.trim(),
          specialty: newProfessionalSpecialty.trim(),
          phone: newProfessionalPhone.replace(/\D/g, ""),
          email: newProfessionalEmail.trim(),
          status: isNewProfessionalActive ? "ativo" : "inativo",
        });
        setStatusNotification({
    ...
    <div className="space-y-6 pb-36">
      <section className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        >
          ...
          <span className="text-sm font-semibold text-white">Criar Profissional</span>
        </button>
      </section>
          ...
          <div className="relative flex-1">
            <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-400 sm:left-3 sm:h-4 sm:w-4" />
              <input
                ...
                className="h-8 w-full rounded-lg border border-slate-200 bg-white pl-8 pr-3 text-xs text-slate-800 shadow-sm placeholder:text-slate-400 focus:border-purple-400 focus:outline-none focus:ring-1 focus:ring-purple-100 sm:h-9 sm:pl-9 sm:pr-4 sm:text-sm"
              />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative" ref={specialtyDropdownRef}>
            <button
              ...
              className="flex h-8 min-w-[8.25rem] items-center justify-between gap-1.5 rounded-lg border border-slate-200 bg-white px-2 py-1 text-left text-xs font-medium text-slate-800 shadow-sm transition hover:border-purple-200 hover:bg-slate-50/80 focus:border-purple-400 focus:outline-none focus:ring-1 focus:ring-purple-100 sm:h-9 sm:min-w-[9.5rem] sm:px-2.5"
            >
              <span className="flex min-w-0 items-center gap-1.5">
                <Filter className="h-3.5 w-3.5 shrink-0 text-purple-500" aria-hidden />
              ...
              >
                {specialtyOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      {active ? (
                        <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-purple-600 text-[8px] font-bold leading-none text-white">
                          ✓
                        </span>
                      ) : (
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            ...
            >
              <header className="mb-4 flex min-w-0 items-start justify-between gap-3">
                <div className="min-w-0 flex-1 overflow-hidden">
                  ...
                  <p
                    className="truncate text-sm font-medium text-purple-700"
              ...
              <footer className="mt-4 flex items-center justify-between border-t border-slate-100 bg-white pt-3">
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="inline-flex items-center gap-1 text-xs font-semibold text-purple-600 transition hover:text-purple-800 sm:text-sm"
                >
                ...
                <div className="flex items-center gap-2 text-slate-400">
                  <button
                    type="button"
                    className="prof-edit-btn text-purple-600"
                  ...
                  >
                    <Pencil
                      className="prof-edit-icon shrink-0 text-slate-500"
                      aria-hidden
                      strokeWidth={2.25}
                    />
                  </button>
                  ...
                  >
                    <Trash2
                      className="prof-delete-icon shrink-0 text-slate-500 hover:text-rose-600"
                      aria-hidden
                      strokeWidth={2.25}
                    />
                  </button>
          ...
          <button
            ...
            className="flex min-h-[220px] w-full flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 text-slate-500 transition hover:border-purple-300 hover:bg-purple-50/40 hover:text-purple-600"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm">
              <UserPlus2 className="h-5 w-5" />
            </div>
            <span className="text-sm font-semibold">Criar Profissional</span>
          </button>
        </div>
        )}
      </section>

      <footer className="pointer-events-none fixed bottom-0 left-0 right-0 z-30 px-4 pb-4 sm:px-6 xl:left-64">
        <section className="pointer-events-auto grid gap-4 rounded-2xl border border-slate-200 bg-white/95 px-4 py-4 text-xs text-slate-500 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] backdrop-blur sm:grid-cols-3 sm:px-6 sm:py-5 sm:text-sm">
          <div>
            ...
            <header className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <div className="flex items-center gap-3 px-1">
                ...
                <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">
                  {drawerMode === "create" ? "Criar Profissional" : "Editar Profissional"}
                </h2>
              </div>
              <button
                type="button"
            ...
            <div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">
              <section className="space-y-3">
                <div className="flex items-center gap-2">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-purple-50 text-xs font-semibold text-purple-600">
                    i
                  </span>
              ...
              <div className="flex gap-3">
                ...
                >
                  <span
                    ...
                    aria-hidden
                  />
                  <span className="relative z-10 transition-colors duration-300 group-hover:text-rose-800">
                    Cancelar
                  </span>
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center px-2 sm:px-6">
          ...
          <div className="relative z-10 flex h-[80vh] w-full max-w-6xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <header className="flex items-center justify-between gap-4 border-b border-slate-200 px-6 py-4">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-purple-50 text-base font-semibold text-purple-700 sm:h-14 sm:w-14 sm:text-lg">
            ...
            <div className="grid flex-1 gap-0 border-t border-slate-100 sm:grid-cols-[260px_minmax(0,1fr)] overflow-y-auto">
              ...
              <section className="flex flex-col gap-2 bg-white p-4 sm:p-6">
                ...
                {scheduleAppointmentsLoading ? (
                  <div className="mt-6 flex items-center justify-center gap-2 text-xs text-slate-500 sm:text-sm">
                  ...
                  <div className="space-y-3">
                    {scheduleAppointments.map((apt) => (
                      <div
                        key={apt.id}
                        className={`rounded-2xl border px-4 py-3 transition-transform hover:scale-[1.01] ${getAppointmentClasses(
                          apt.status
                        )}`}
                      >
                        ...
                        <p className="mt-0.5 text-sm font-bold text-slate-900">{apt.patient}</p>
                        <p className="text-xs font-medium text-slate-500">{apt.procedure}</p>
                      </div>
                    ))}
                  </div>
                ) : (
            ...
            <footer className="flex items-center justify-between gap-4 border-t border-slate-200 bg-slate-50 px-6 py-3 text-xs text-slate-600 sm:text-sm">
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    <span className="font-semibold text-slate-800">
                      {String(scheduleAppointments.length).padStart(2, "0")}
                    </span>{" "}
                    Agendamentos
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-full bg-purple-600 px-5 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-purple-700 sm:text-sm"
                >
                  Fechar
                </button>
              </div>
      ...
      {isDeleteModalOpen && selectedProfessionalToDelete ? (
        <div className="fixed inset-0 z-40 flex items-center justify-center px-4 sm:px-6">
          ...
          />
          <div className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-slate-200 bg-white">
            ...
            <div className="px-5 py-5 sm:px-6">
              <p className="text-sm leading-relaxed text-slate-600">
                ...
                <span className="font-semibold text-slate-900">{selectedProfessionalToDelete.name}</span>
                ? Esta ação não poderá ser desfeita.
              </p>
              {deleteError ? (
                <p className="mt-4 rounded-xl border border-rose-100 bg-rose-50 px-3 py-2.5 text-sm text-rose-800">
                  {deleteError}
                </p>
              ) : null}
            </div>
            <div className="flex items-center justify-end gap-3 border-t border-slate-200 px-5 py-4 sm:px-6">
              <button
                ...
                className="inline-flex min-w-[110px] items-center justify-center rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-xs font-semibold text-slate-600 shadow-sm transition hover:bg-slate-100 hover:text-slate-800 disabled:opacity-50 sm:text-sm"
              >
                Cancelar
              </button>
              <button
                type="button"
                disabled={deleteLoading}
                className="inline-flex min-w-[110px] items-center justify-center rounded-lg bg-rose-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm transition hover:bg-rose-700 disabled:opacity-70 sm:text-sm"
                onClick={async () => {
